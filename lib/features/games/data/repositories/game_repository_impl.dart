...
class GameRepositoryImpl implements GameRepository {
    ...
    try {
      ...
      try {
        LoggerService.instance.log(
          '[GameRepository] Checking Heroic config: ${_platformService.gameConfigPath}',
        );
      ...
      try {
        LoggerService.instance.log(
          '[GameRepository] Checking OGI library: ${_platformService.ogiLibraryPath}',
        );
      ...
      try {
        LoggerService.instance.log(
          '[GameRepository] Checking Lutris config: ${_platformService.lutrisConfigPath}',
        );
  ...
  Future<Result<Unit>> applyLsfgToGames(List<String> appNames) async {
    ...
    for (final appName in appNames) {
      try {
        ...
        if (e is GameNotInSteamException) {
          return Left(GameNotInSteamFailure(appName));
        }
        LoggerService.instance.log('[GameRepository] Failed to apply LSFG for $appName: $e');
        LoggerService.instance.log('[GameRepository] Stack trace: $stackTrace');
        return Left(FileSystemFailure('Failed to apply LSFG for $appName: $e'));
      }
    }
  ...
  Future<void> _modifyGameConfig(String id, {required bool addLsfg}) async {
    ...
    LoggerService.instance.log(
      '[GameRepository] _modifyGameConfig: src=$source, app=$appName (addLsfg: $addLsfg)',
    );
    ...
    if (source == 'heroic') {
      ...
      try {
        ...
        if (e is GameNotInSteamException) {
          rethrow; // Propagate to be caught by applyLsfgToGames
        }
        LoggerService.instance.log(
          '[GameRepository] Failed to find OGI game details for id: $appName. Error: $e',
        );
  ...
  Future<void> _modifyHeroicConfig(String appName, {required bool addLsfg}) async {
    ...
    // Create backup
    final backupFile = File('${heroicFile.path}.bak');
    await backupFile.writeAsString(content);
    LoggerService.instance.log('[GameRepository] Created backup: ${backupFile.path}');

    final json = jsonDecode(content) as Map<String, dynamic>;

    // Handle "enviromentOptions" typo by checking keys but PRESERVING the one found
    ...
    if (json.containsKey(keyToUse) && json[keyToUse] is Map) {
  ...
  Future<void> _modifyLutrisConfig(String appName, {required bool addLsfg}) async {
    ...
    if (await lutrisFile.exists()) {
      ...
      try {
        ...
        // Create backup
        final backupFile = File('${lutrisFile.path}.bak');
        await backupFile.writeAsString(content);
        LoggerService.instance.log('[GameRepository] Created backup: ${backupFile.path}');
        ...
        if (yaml is Map) {
          ...
          if (!yaml.containsKey('system') || yaml['system'] == null) {
            ...
            // Ensure 'env' node exists under 'system'
            final system = yaml['system'];
            if (system is Map && (!system.containsKey('env') || system['env'] == null)) {
              doc.update(['system', 'env'], {});
            } else if (system is! Map) {
              // e.g. system is string or something else, force overwrite?
              // Safer to recreate
              doc.update(['system'], {'env': {}});
            }
          }
        } else {
          // Empty yaml or list? Reset to map
          doc.update([], {
            'system': {'env': {}},
          });
        }
        ...
        if (addLsfg) {
          ...
          // Only try to remove if it exists to avoid errors
          try {
            final currentEnv = ((yaml as Map)['system'] as Map)['env'];
            if (currentEnv is Map && currentEnv.containsKey(PlatformService.lsfgEnvKey)) {
              doc.remove(['system', 'env', PlatformService.lsfgEnvKey]);
            }
          } catch (_) {
            // Ignore if path lookup fails, means key isn't there
          }
        }
