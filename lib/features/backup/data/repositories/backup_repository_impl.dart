...
class BackupRepositoryImpl implements BackupRepository {
  ...
  Future<Result<Backup>> createBackup() async {
    try {
      ...
      if (steamShortcuts.isNotEmpty) {
        ...
        LoggerService.instance.log('[BackupRepository] Backed up ${steamShortcuts.length} Steam shortcuts files');
      }
      ...
      LoggerService.instance.log('[BackupRepository] Backup complete: $backupName ($filesCopied files)');
        ...
        if (await steamBackup.exists()) {
          await for (final entity in steamBackup.list()) {
            if (entity is File && entity.path.endsWith('.vdf')) {
              ...
              final targetPath = '${_platformService.steamUserDataPath}/$userId/config/shortcuts.vdf';
              
              final targetFile = File(targetPath);
              if (await targetFile.parent.exists()) {
                await entity.copy(targetPath);
                filesRestored++;
                LoggerService.instance.log('[BackupRepository] Restored shortcuts for user $userId');
              }
            }
          }
        }
  ...
  Future<List<File>> _findShortcutsFiles() async {
    ...
    try {
      ...
      LoggerService.instance.log('[BackupRepository] Error finding Steam users: $e');
    }
    return results;
  }
}
